...
| `status` | Show all projects' state and reservations |
| `reserve [<platform> <id>] [--label <name>] [--list]` | Mark an external sim/emulator as in-use so rn-iso skips it |
| `unreserve [<id\|label>] [--all]` | Release a reservation |
| `release [<project\|label\|udid>] [--platform <p>]` | Unbind a project assignment or reservation |
| `shutdown [--platform <p>]` | Release and shut down sims for current project |
| `prune [--shutdown]` | GC dead entries machine-wide |
| `logs` | Tail Metro log for current project |
...
npx rn-iso reserve                # interactive multi-select picker
npx rn-iso unreserve agent-1      # release by label, UDID, or serial
```

Reservations show up tagged `[reserved]` in the picker. They're selectable, but picking one prompts you to confirm taking it over (which removes the reservation).
