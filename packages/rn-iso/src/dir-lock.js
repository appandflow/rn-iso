...
function acquire(lockPath, { staleMs, waitMs, pollMs, ensureParent }) {
  ensureParent?.();
...
export function withDirLock(lockPath, fn, { staleMs = DEFAULT_STALE_MS, waitMs = DEFAULT_WAIT_MS, pollMs = DEFAULT_POLL_MS, ensureParent } = {}) {
  ...
  if (depth > 0) {
    ...
    try {
      ...
      depths.set(lockPath, depths.get(lockPath) - 1);
    }
  }
