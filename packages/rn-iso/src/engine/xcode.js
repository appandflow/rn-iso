...
import { cleanLine, createLineReader } from '../supervisor/server-expo.js';
...
function buildFailure(message, remedy) {
...
export function pickXcodeProject(entries) {
...
// programmer error.
export function discoverXcodeProject(root) {
...
export function parseSchemeList(text) {
  ...
  const schemes = Array.isArray(container.schemes)
    ? container.schemes.filter(s => typeof s === 'string' && s.trim() !== '')
    : [];
...
export function pickScheme(schemes, containerName) {
  const list = (Array.isArray(schemes) ? schemes : []).filter(s => typeof s === 'string' && s.trim() !== '');
...
export function listSchemes(project, { exec = null } = {}) {
  const executor = exec || getExecutor();
  try {
    const out = executor.runFile(
      'xcodebuild',
      [project.flag, project.path, '-list', '-json'],
...
export function resolveScheme(project, { exec = null } = {}) {
...
export function productsDir(derivedDataPath, { configuration = 'Debug', sdk = 'iphonesimulator' } = {}) {
...
export function pickAppBundle(entries, preferredName = null) {
...
export function findAppBundle(dir, preferredName = null) {
  let entries;
  try {
    entries = readdirSync(dir);
  } catch {
...
export function parseBundleId(plistJson) {
...
export function readBundleId(appPath, { exec = null } = {}) {
  const executor = exec || getExecutor();
  try {
...
// absurd.
export function tailLines(lines, count = 5) {
  const nonEmpty = (Array.isArray(lines) ? lines : []).filter(l => typeof l === 'string' && l.trim() !== '');
  return nonEmpty.slice(-count);
}

function failedResult({ code, diagnostics, durationMs, exitCode = null, transcriptLines = 0, tail = [] }) {
...
export async function buildIos({
  ...
  const reportError = (message, remedy) => {
  ...
  if (!target) {
    ...
    if (discovered.error) {
      ...
      return failedResult({
        ...
        diagnostics: [{ message: discovered.error.message, remedy: discovered.error.remedy }],
        durationMs: elapsed(),
      });
    }
    target = discovered;
  ...
  if (!chosenScheme) {
    const resolved = resolveScheme(target, { exec: executor });
    if (resolved.error) {
  ...
  try {
    ...
    const message = `Could not run xcodebuild: ${err.message}`;
  ...
  const appPath = findAppBundle(products, chosenScheme);
  if (!appPath) {
