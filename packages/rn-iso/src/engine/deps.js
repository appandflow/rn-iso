...
export function podsAreStale(lockText, manifestText) {
...
export function podfilePath(root) {
...
export function readPodState(root) {
  return {
...
export async function runPodInstall(root, logWriter, { spawnFn = null, now = Date.now } = {}) {
  ...
  try {
    ...
    return missingPod(err) || {
      failed: true,
      code: DEPS_ERROR,
      reason: `Could not run \`pod install\`: ${err?.message || err}`,
      lastLines: [],
    };
  }
...
export function missingPod(err) {
  const message = String(err?.message || err || '');
  if (err?.code !== 'ENOENT' && !/ENOENT|not found/i.test(message)) return null;
  return {
    failed: true,
    code: DEPS_ERROR,
