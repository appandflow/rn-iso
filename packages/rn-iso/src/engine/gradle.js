...
import { createLineReader, stripAnsi } from '../supervisor/server-expo.js';
import { waitForChild } from './deps.js';
import { capDiagnostics, extractGradleDiagnostics } from './errors-gradle.js';
...
export function androidDir(root) {
  return join(root, 'android');
}

export function gradlewPath(root) {
  return join(androidDir(root), 'gradlew');
}

export function debugApkDir(root) {
...
export function discoverAndroidProject(root) {
...
export function androidSdkRefusal({ sdkPath, sdkExists, hasLocalProperties }) {
...
export function pickDebugApk(files) {
...
export function parseOutputMetadata(text) {
  let parsed;
  try {
  ...
  const files = elements.map((e) => e?.outputFile).filter((f) => typeof f === 'string');
...
export function parseApkFromTranscript(text) {
...
export function locateDebugApk(root, transcript = '') {
...
export async function buildAndroid({ root, logWriter }, { spawnFn = null, now = Date.now, env = process.env } = {}) {
  const project = discoverAndroidProject(root);
  if (project.failed) return { ...project, diagnostics: [], truncated: 0, lastLines: [], durationMs: 0 };
  ...
  if (refusal) return { failed: true, ...refusal, diagnostics: [], truncated: 0, lastLines: [], durationMs: 0 };
  ...
  let child;
  try {
    child = spawn(project.gradlew, [ASSEMBLE_TASK], {
...
function spawnFailure(err, project, durationMs) {
  const message = String(err?.message || err || '');
  const permissionDenied = err?.code === 'EACCES' || /EACCES|permission denied/i.test(message);
  return {
    failed: true,
    code: BUILD_ERROR,
