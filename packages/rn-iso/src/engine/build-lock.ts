...
// The holder's record, as written to lock.json and read back by every caller.
...
// FLAT rather than a discriminated union: acquireBuildLock returns exactly one
// of `acquired`+`path`+`lock`, or `held`+`path` -- see the doc comment below.
...
// waitForBuild for the three outcomes this covers.
