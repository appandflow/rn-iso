// src/ndjson.js -- Contract 1: the log record, and the two ends that touch it.
//
...
export function formatNdjsonLine(record) {
  try {
...
export function createNdjsonWriter(file) {
  let fd = null;
  let written = 0;
  let dropped = 0;
  let lastError = null;
...
function stamp(record) {
  const base = record && typeof record === 'object' && !Array.isArray(record) ? record : { msg: String(record) };
