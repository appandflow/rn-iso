...
export function projectRequire(root) {
...
export function normalizeModule(mod, names) {
  if (!mod || typeof mod !== 'object') return mod;
  const has = (obj) => obj && names.every((n) => typeof obj[n] === 'function');
...
export function checkBareApi(modules) {
  const problems = [];
...
export function resolveBareDeps(root, { requireFrom = projectRequire } = {}) {
  ...
  for (const name of BARE_PACKAGES) {
    ...
    try {
      ...
      throw supervisorError(
        'RN_ISO_BARE_LOAD',
        `${name} is installed in ${root} but failed to load: ${err?.message || err}`,
...
export function loadNdjsonReporter(root, { requireFrom = createRequire } = {}) {
...
// it changed anything.
export function ensureNativePlatform(config) {
...
export function reporterLogger(reporter) {
  const at = (level) => (...data) => {
    try {
...
function closeHttpServer(httpServer, timeoutMs) {
