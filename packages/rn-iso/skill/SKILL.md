...
- **`launched: "unverified"` in the `--json` payload is not success.** The app started but never fetched a bundle from this workspace's Metro -- usually the expo-dev-client server picker waiting for a tap (it lists EVERY Metro on the machine; tapping another workspace's row loads the wrong project's bundle), or iOS gating the deep link behind an "Open in app?" alert. The stderr warning names the exact `simctl openurl` command; drive the picker or alert with your device tool, and pick THIS workspace's port from the facts, never a row by memory.
