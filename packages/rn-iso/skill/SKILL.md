...
```bash
cd "$(npx rn-iso worktree create app-412 --carry-ignored)"   # 1. isolated worktree
...
`npx rn-iso init` writes the rest of that setup into the repo (`.rn-iso/` into `.gitignore`, an executable `scripts/dev`) and reports what is still missing -- the Metro cache stores and the compilation-cache CAS pin are findings it reports rather than edits it makes, because both live in files the project already owns. `npx rn-iso doctor` reports the same findings on their own, read-only, so you can inspect a repo you do not own without modifying it. **Read the repo's own `scripts/dev` first if it has one** -- it may wrap rn-iso in steps this repo needs, and it wins over anything you would invent. (init generated a `WORKFLOW.md` in earlier versions and no longer does; a repo that still has one is the repo's own document, not a description of this version.)
...
`init` (set a repo up for parallel agents) · `doctor` (what is silently costing build time) · `worktree create|remove` · `start` (dev server under a supervisor, blocks until healthy) · `stop` (the inverse of `start`) · `ios` · `android` (build/install/launch, from cache when nothing native changed) · `logs` (query the captured NDJSON timeline) · `status` (devices, ports, supervisor health, last build, error counts) · `gc` (dead entries, orphaned devices, and the shared build caches) · `guide` · `skill install`

That is all of it. There is no `up`, no `release`, no `shutdown`, no `config`, no `build-cache`, no `worktree list` -- `status` covers the last one, `ios`/`android` absorbed the build cache, settings are files (`guide settings`), and destruction consolidated into `worktree remove` and `gc --delete`. Run `npx rn-iso <command> --help` for flags, or `npx rn-iso guide` for the reference.
