...
describe('option parsing', () => {
  test('--wait defaults to 60 seconds', () => {
    assert.deepEqual(parseWait(undefined), { seconds: 60 });
  });

  test('--wait takes a number of seconds', () => {
    assert.deepEqual(parseWait('90'), { seconds: 90 });
  });

  test('a --wait that is not a positive number is refused, not silently defaulted', () => {
    assert.match(parseWait('soon').error, /--wait/);
    assert.match(parseWait('0').error, /--wait/);
    assert.match(parseWait('-5').error, /--wait/);
  });
});
...
describe('liveSupervisor', () => {
  ...
  test('takes the workspace state first, because only it carries the mode', () => {
    ...
    assert.deepEqual(found, { pid: 10, port: 8082, mode: 'bare-inproc', startedAt: 'T' });
  });
  ...
  test('no record at all is null, not a throw', () => {
    assert.equal(liveSupervisor({ port: 8082 }), null);
    assert.equal(liveSupervisor(), null);
  });
});
...
describe('startFacts', () => {
  test('shapes the facts an agent reads', () => {
    assert.deepEqual(
      startFacts({ port: 8082, supervisor: { pid: 91, mode: 'expo-child' }, logsDir: '/w/.rn-iso/logs', alreadyRunning: false }),
      { port: 8082, supervisorPid: 91, mode: 'expo-child', logsDir: '/w/.rn-iso/logs', alreadyRunning: false }
    );
  });
  ...
  test('a dev server rn-iso did not start reports a null supervisor rather than a lie', () => {
    ...
    assert.equal(facts.supervisorPid, null);
    assert.equal(facts.mode, null);
    assert.equal(facts.alreadyRunning, true);
  });
});
...
describe('the supervisor log tail', () => {
  ...
  test('a log that does not exist yet is an empty tail, not a throw', () => {
    assert.deepEqual(readLogTail(join(root, 'nope.log')), []);
  });
});
...
describe('action: already running', () => {
  test('a healthy dev server with a live supervisor record is a no-op exit 0', async () => {
    ...
    assert.equal(result.exitCode, null, 'an already-running dev server is success');
    assert.equal(result.logs.length, 1, `exactly one stdout line, got ${JSON.stringify(result.logs)}`);
  ...
  test('a healthy dev server the agent started itself is reported, not fought', async () => {
    ...
    assert.deepEqual(exec.calls.spawn, [], 'starting a second bundler over a working one is the actual failure');
  ...
  test('two starts in a row leave one supervisor', async () => {
    ...
    assert.deepEqual(exec.calls.spawn, []);
  });
});
...
describe('action: spawning the supervisor', () => {
  test('spawns node run.js --root --port detached, with stdio into supervisor.log, and waits for health', async () => {
    ...
    assert.equal(result.exitCode, null, `expected success, stderr: ${result.errs.join('\n')}`);
    const spawned = exec.calls.spawn[0];
    assert.equal(spawned.cmd, process.execPath);
    assert.deepEqual(spawned.args, [supervisorEntry(), '--root', root, '--port', String(port)]);
    assert.equal(spawned.opts.cwd, root, 'the identity check reads the holder\'s cwd');
    assert.equal(spawned.opts.detached, true, 'the supervisor must outlive this command');
    assert.equal(spawned.opts.stdio[0], 'ignore');
    assert.equal(typeof spawned.opts.stdio[1], 'number', 'stdout is an fd on supervisor.log');
    assert.equal(spawned.opts.stdio[1], spawned.opts.stdio[2], 'stdout and stderr share the fd so they interleave');
    assert.ok(existsSync(supervisorLogFile(root)));

    const facts = JSON.parse(result.logs[0]);
    assert.equal(facts.port, port);
    assert.equal(facts.alreadyRunning, false);
    assert.equal(facts.supervisorPid, process.pid);
    assert.equal(facts.mode, 'bare-inproc', 'the mode comes from the state file the supervisor wrote');
  });
  ...
  test('a supervisor that never answers exits 1 with the log tail and the log path', async () => {
    ...
    assert.match(stderr, new RegExp(supervisorLogFile(root).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
    assert.ok(!stderr.includes('noise 1'), 'the tail is five lines, not the whole transcript');
    assert.ok(stderr.includes('noise 4'));
  });
...
describe('action: the reserved port', () => {
  test('a FOREIGN holder of the reserved port moves the reservation instead of counting as healthy', async () => {
    ...
    assert.notEqual(reserved, port, 'the reservation must move off a foreign holder');
    assert.match(result.errs.join('\n'), /is held by something else/);
    // It then tries to start on the new port, and fails to come up within 1s.
    assert.equal(result.exitCode, 1);
    assert.equal(exec.calls.spawn[0].args[4], String(reserved));
  });
  ...
  test('a project with no reservation gets one', async () => {
    ...
    const reserved = getProject(root).metroPort;
    assert.equal(typeof reserved, 'number');
    assert.equal(exec.calls.spawn[0].args[4], String(reserved));
    assert.equal(result.exitCode, 1, 'nothing came up, so this run still fails');
  });
...
describe('action: an existing supervisor that is not answering', () => {
  test('waits on the one that exists rather than spawning a second', async () => {
    ...
    assert.deepEqual(exec.calls.spawn, [], 'two supervisors for one workspace is the failure to avoid');
    assert.equal(result.exitCode, 1);
    assert.match(result.errs.join('\n'), /did not serve port 8158/);
    assert.match(result.errs.join('\n'), /rn-iso stop/);
  });
...
describe('output contract', () => {
  test('without --json every line is human and nothing is JSON', async () => {
    ...
    assert.equal(result.exitCode, null);
    assert.match(result.logs.join('\n'), /OK: dev server on port 8160/);
    assert.match(result.logs.join('\n'), /expo-child/);
    assert.match(result.logs.join('\n'), new RegExp(workspaceLogsDir(root).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
    for (const line of result.logs) {
      assert.throws(() => JSON.parse(line), 'human mode must not emit JSON');
    }
  });
});
