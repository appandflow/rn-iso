...
describe('tryAcquireBuildSlot', () => {
  ...
  test('returns null when every slot is held by a LIVE builder', () => {
    ...
    assert.equal(tryAcquireBuildSlot({ max: 2, isAlive: alwaysAlive }), null);
  });
...
describe('releaseBuildSlot', () => {
  test('removes our slot', () => {
    ...
    assert.equal(releaseBuildSlot(got), true);
  ...
  test('refuses to remove a slot another pid now holds', () => {
    ...
    assert.equal(releaseBuildSlot(got), false);
    assert.ok(existsSync(got.path), 'the new holder\'s slot must survive our release');
  });

  test('an unlimited handle releases to a no-op', () => {
    assert.equal(releaseBuildSlot({ acquired: true, unlimited: true }), false);
  });
});
...
describe('acquireBuildSlot', () => {
  test('unlimited (max 0) acquires immediately without a slot on disk', async () => {
    ...
    assert.equal(got.unlimited, true);
    assert.equal(existsSync(buildSlotsDir()), false);
  });
});
...
describe('listBuildSlots', () => {
  test('classifies slots by whether the builder is alive', () => {
    ...
    assert.equal(byIndex[0].alive, true);
    assert.equal(byIndex[1].alive, false);
    assert.equal(byIndex[1].projectRoot, '/w/dead');
  });
});
...
describe('live: 2 slots, 3 processes', () => {
  test('the third process waits for a slot and gets one on release', async () => {
    ...
    assert.ok(await waitFor('acquired-A'), 'A must acquire');
    assert.ok(await waitFor('acquired-B'), 'B must acquire');

    const c = spawn('C');
    // With both slots full, C cannot have acquired yet.
    await new Promise(r => setTimeout(r, 400));
    assert.equal(existsSync(join(dir, 'acquired-C')), false, 'C must WAIT while both slots are full');

    // Free one slot; C should now get in.
    writeFileSync(join(dir, 'release-A'), '1');
    assert.ok(await waitFor('acquired-C'), 'C must acquire once A releases');
    ...
    assert.ok(acquiredC >= releasedA, 'C acquired only after A released its slot');
