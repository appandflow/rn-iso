...
describe('podsAreStale', () => {
  test('identical lock and manifest are not stale', () => {
    assert.deepEqual(podsAreStale(LOCK, LOCK), { stale: false });
  });
  ...
  test('a missing Manifest.lock (no Pods installed) is stale', () => {
    ...
    assert.equal(result.stale, true);
    assert.match(result.reason, /Manifest\.lock is missing/);
  });

  test('a Pods sandbox with no Podfile.lock describing it is stale', () => {
    const result = podsAreStale(null, LOCK);
    assert.equal(result.stale, true);
    assert.match(result.reason, /Podfile\.lock is missing/);
  });
  ...
  test('neither file present means no CocoaPods here, which is NOT stale', () => {
    assert.deepEqual(podsAreStale(null, null), { noPods: true, stale: false });
    assert.deepEqual(podsAreStale(undefined, undefined), { noPods: true, stale: false });
  });
  ...
  test('line-ending and trailing-whitespace differences are not a dependency change', () => {
    assert.deepEqual(podsAreStale(LOCK, LOCK.replace(/\n/g, '\r\n')), { stale: false });
    assert.deepEqual(podsAreStale(LOCK, `${LOCK}\n\n`), { stale: false });
  });
});
...
describe('readPodState', () => {
  test('reports the two files and the Podfile, with absent files as null', () => {
    ...
    assert.deepEqual(readPodState(root), { hasPodfile: false, lockText: null, manifestText: null });
    ...
    assert.equal(state.hasPodfile, true);
    assert.deepEqual(podsAreStale(state.lockText, state.manifestText), { stale: false });
  });
});
...
describe('runPodInstall', () => {
  test('runs `pod install` with cwd ios/ and streams the transcript as build/debug records', async () => {
    ...
    assert.equal(spawned.cmd, 'pod');
    assert.deepEqual(spawned.args, ['install']);
    assert.equal(spawned.opts.cwd, join(root, 'ios'));
    assert.deepEqual(
      writer.records.map(r => [r.src, r.level, r.msg]),
      [['build', 'debug', 'Analyzing dependencies'], ['build', 'debug', 'Pod installation complete!']]
    );
  });
  ...
  test('a non-zero exit comes back as {failed, lastLines}, never a throw', async () => {
    ...
    assert.equal(result.code, DEPS_ERROR);
  ...
  test('a missing `pod` binary is reported as a structured remedy, not an ENOENT', async () => {
    ...
    assert.equal(result.failed, true);
    assert.equal(result.code, DEPS_ERROR);
    assert.match(result.reason, /CocoaPods is not installed/);
    assert.match(result.remedy, /brew install cocoapods/);
  });
  ...
  test('refuses when there is no ios/ directory at all', async () => {
    ...
    assert.equal(result.failed, true);
    assert.match(result.reason, /No ios\/ directory/);
  });
});
