...
describe('parseArgs', () => {
  test('accepts the ios invocation and derives the predicate app name from the bundle id', () => {
    assert.deepEqual(parseArgs(['--platform', 'ios', '--root', '/abs', '--udid', 'U1', '--bundle', 'com.example.MyApp']), {
      platform: 'ios', root: '/abs', udid: 'U1', bundleId: 'com.example.MyApp', appName: 'MyApp', serial: null, packageName: null,
    });
  });

  test('an explicit --app-name wins, because the product name is not always the last bundle segment', () => {
    assert.equal(parseArgs(['--platform', 'ios', '--root', '/abs', '--udid', 'U1', '--bundle', 'com.example.app', '--app-name', 'MyApp']).appName, 'MyApp');
  });

  test('accepts the android invocation', () => {
    const parsed = parseArgs(['--platform', 'android', '--root', '/abs', '--serial', 'emulator-5554', '--package', 'com.example.app']);
  ...
  test('refuses a missing or unknown platform, a relative root, and each missing per-platform argument', () => {
    assert.match(parseArgs([]).error, /--platform/);
    assert.match(parseArgs(['--platform', 'web', '--root', '/abs']).error, /--platform/);
    assert.match(parseArgs(['--platform', 'ios', '--root', 'rel']).error, /absolute/);
    assert.match(parseArgs(['--platform', 'ios', '--root', '/abs']).error, /--udid/);
    assert.match(parseArgs(['--platform', 'ios', '--root', '/abs', '--udid', 'U']).error, /--bundle/);
    assert.match(parseArgs(['--platform', 'android', '--root', '/abs']).error, /--serial/);
    assert.match(parseArgs(['--platform', 'android', '--root', '/abs', '--serial', 'S']).error, /--package/);
  });

  test('refuses an unknown argument rather than ignoring it', () => {
    assert.match(parseArgs(['--platform', 'ios', '--root', '/abs', '--follow']).error, /Unknown/);
  });
});
...
describe('Contract 5: the registration', () => {
  test('registers under collectors.<platform> without disturbing the supervisor record', () => {
    ...
    assert.deepEqual(state().collectors, { ios: { pid: 999, startedAt: 'now' } });
    assert.deepEqual(state().supervisor, { pid: 123, port: 8082 });
  });
  ...
  test('two platforms coexist, and unregistering one leaves the other', () => {
    ...
    assert.deepEqual(Object.keys(readCollectors(root)).sort(), ['android', 'ios']);
    unregisterCollector(root, 'ios');
    assert.deepEqual(readCollectors(root), { android: { pid: 2, startedAt: 'b' } });
  });
  ...
  test('the last collector out removes the key entirely rather than leaving an empty object', () => {
    ...
    assert.deepEqual(state().supervisor, { pid: 123 });
  });
...
describe('the ios collector, spawned for real against a fake xcrun', () => {
  test('registers its own pid, writes records, and clears the registration on SIGTERM', async () => {
    ...
    const child = spawnCollector(['--platform', 'ios', '--root', root, '--udid', 'UDID-1', '--bundle', 'com.example.MyApp']);

    const registered = await until(() => readCollectors(root).ios, { label: 'the collector registration' });
    assert.equal(registered.pid, child.pid, 'the registration must carry the collector process pid');
    assert.match(registered.startedAt, /^\d{4}-\d{2}-\d{2}T/);

    const records = await until(
      () => { const r = deviceLog().filter(x => x.level === 'fatal'); return r.length ? r : null; },
      { label: 'a parsed device record' }
    );
    assert.match(records[0].msg, /LocationProvider/);
    assert.equal(records[0].proc, 'locationd');
    assert.equal(deviceLog()[0].event, 'collector_started');

    process.kill(child.pid, 'SIGTERM');
    const result = await exited(child);
    assert.deepEqual(result, { code: 0, signal: null }, 'a signalled collector exits cleanly');
    assert.equal('collectors' in (state() || {}), false, 'the registration must not outlive the process');
    assert.ok(deviceLog().some(r => r.event === 'collector_stopped'), 'no exit path is silent');
  });
  ...
  test('survives the stream ending: a record, and exit 0', async () => {
    writeShim('xcrun', [
      ...iosShimLines().slice(0, 1).map(l => `cat <<'LINE'\n${l}\nLINE`),
      'exit 0',
    ].join('\n'));
    ...
    assert.ok(stopped, 'the stream ending must be recorded');
    assert.match(stopped.msg, /device log stream ended/);
    assert.equal('collectors' in (state() || {}), false);
  });
});
...
describe('the android collector, spawned for real against a fake adb', () => {
  test('resolves the app pid, filters logcat on it, and cleans up on SIGTERM', async () => {
    ...
    const child = spawnCollector(['--platform', 'android', '--root', root, '--serial', 'emulator-5554', '--package', 'com.example.app']);
    ...
    const errors = await until(
      () => { const r = deviceLog().filter(x => x.level === 'error'); return r.length ? r : null; },
      { label: 'a parsed logcat record' }
    );
    assert.match(errors[0].msg, /undefined is not a function/);
    assert.equal(errors[0].proc, 'ReactNativeJS(3132)');
    // The banner is skipped, not recorded raw.
    assert.equal(deviceLog().some(r => r.msg?.includes('beginning of main')), false);
  ...
  test('is registered while it is still waiting for the app pid to appear', async () => {
    ...
    const registered = await until(() => readCollectors(root).android, { label: 'the android registration' });
    assert.equal(registered.pid, child.pid);
    assert.equal(deviceLog().some(r => r.event === 'collector_started'), false, 'nothing is streaming yet');
...
describe('runCollector seams', () => {
  test('an app whose pid never appears is an error record, exit 1, and no registration left behind', async () => {
    ...
    const result = await runCollector({
      ...
      startStream: () => { throw new Error('must not start logcat without a pid'); },
      attachSignals: false,
      onExit: (c) => { code = c; },
    });
    ...
    assert.match(failed.msg, /no process appeared/);
  ...
  test('a stream that fails to start is an error record rather than a throw', async () => {
    ...
    const result = await runCollector({
      ...
      udid: 'U1',
      appName: 'MyApp',
      startStream: () => { throw new Error('spawn xcrun ENOENT'); },
      attachSignals: false,
      onExit: (c) => { code = c; },
    });
    assert.equal(result, null);
