...
- **`android` refused with `STIM_CLI_NO_DEVICE`.** Read the message before touching anything: the emulator's own output is captured in the global workspace `logs/emulator.log` (truncated per boot).
