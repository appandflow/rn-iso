...
**Setting a repo up is not a step.** rn-iso runs on a clean checkout: the Xcode
compilation cache, Gradle's build cache and a shared Metro transform store all
ride on the command lines rn-iso composes itself. When something IS blocked or
slow, `rn-iso doctor` is the read-only second opinion:

```bash
npx rn-iso doctor
```

It reports only what rn-iso cannot handle on its own -- a missing dev client,
...
the settings that matter solely for builds you make outside rn-iso. There is no
`rn-iso init` and no setup skill: the edits doctor names land in files the
...
ignoring `.rn-iso/`, is self-ensured: `start`, `ios` and `android` each add the
entry if git does not already ignore the directory, and say so once on stderr.
It goes in this clone's `.git/info/exclude` rather than the project's
`.gitignore` -- git honours the two identically, but the exclude file is
per-clone and never tracked, so **nothing needs committing and no tracked file
is ever modified**. A project that already ignores `.rn-iso/` is left alone, and
outside a git repo nothing is written at all.
