...
There is no machine-wide daemon: one supervisor process per workspace, recorded in `<root>/.stim-cli/state.json` and in the registry before it starts serving, and gone (with its records) on any exit path. Two modes:
...
Everything lands as one JSON object per line under `<root>/.stim-cli/logs`, and `stim-cli logs` queries the files merged into one timeline:
