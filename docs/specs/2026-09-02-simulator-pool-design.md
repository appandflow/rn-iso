...
its owned simulator, Stim parks it instead of deleting it: shut down,
uninstall the workspace's app, reset the app's privacy grants and the
keychain, rename, record. When a workspace needs a simulator, Stim adopts a
...
about 21 s, so erasing costs most of what parking saves. Not erasing keeps
the previous workspace's app data, privacy grants, and keychain items, which
is why parking removes those three things explicitly.
...
2. `simctl uninstall <udid> <bundleId>` for the workspace's recorded app
...
- Real simctl once (invariant 9): the park recipe (`uninstall`, `privacy
reset all`, `keychain reset`, `rename`) and the adopt rename, with the
  measured boot time after the recipe recorded in the PR (expected about
  9 s, not the erase path's 21 s).
