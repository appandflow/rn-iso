# rn-iso — hand Metro back to the agent, keep the port
...
The broker principle from the spawn-and-reap spec has one carve-out: rn-iso
...
script is `react-native start --client-logs`, and rn-iso spawned
...
while rn-iso's managed Metro sits idle. The failure mode managed Metro
...
- `src/commands/logs.js` and per-project Metro log files (nothing writes
  them once rn-iso does not own the process's stdio).
...
- `src/ports.js` allocation and reclamation, unchanged. This is the whole
  reason rn-iso still touches Metro at all.
...
- "Metro is always managed by rn-iso ... never start your own Metro" replaced
  with "start it yourself, on the port rn-iso assigned."
...
by an earlier one no longer has `rn-iso logs` to find its output. Mitigated,
