# Stim — hand Metro back to the agent, keep the port

Date: 2026-08-20
Status: draft
Amends: `2026-08-16-spawn-and-reap-broker-design.md`. That spec made Stim a
...
The broker principle from the spawn-and-reap spec has one carve-out: Stim
...
script is `react-native start --client-logs`, and Stim spawned
...
while Stim's managed Metro sits idle. The failure mode managed Metro
...
- `src/commands/logs.js` and per-project Metro log files (nothing writes
  them once Stim does not own the process's stdio).
...
- `src/ports.js` allocation and reclamation, unchanged. This is the whole
  reason Stim still touches Metro at all.
...
Today the recorded PID _is_ proof of provenance, because Stim spawned the
...
Metros that Stim never started, which today leak entirely.
...
- "Metro is always managed by Stim ... never start your own Metro" replaced
  with "start it yourself, on the port Stim assigned."
...
by an earlier one no longer has `stim logs` to find its output. Mitigated,
...
SKILL.md tells agents "Metro is always managed by Stim ... never start your
