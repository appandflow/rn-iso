...
```bash
npx rn-iso@latest --version     # must be >= 1.0.0; npx caches stale versions
npx rn-iso skill install        # then READ the installed skill before starting
npx rn-iso guide lifecycle      # generated by the binary; cannot be stale
```

If this machine ran rn-iso v2 (0.x): BEFORE anything else, run `npx rn-iso
status` and `npx rn-iso gc` (report-only) and read both skeptically — v3 was
never guaranteed against v2-written state. Old cache dirs
...
2. **Workspace.** `cd "$(npx rn-iso worktree create <name> --carry-ignored)"`.
...
- CRITICAL: the documented loop cannot complete unassisted, or rn-iso
