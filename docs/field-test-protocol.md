...
```bash
npx stim-cli@latest --version     # must be >= 1.0.0; npx caches stale versions
npx skills add appandflow/stim-cli   # then READ the installed skill before starting
npx stim-cli guide lifecycle      # generated by the binary; cannot be stale
```

If this machine ran an earlier build of stim-cli: BEFORE anything else, run `npx stim-cli
status` and `npx stim-cli gc` (report-only) and read both skeptically — compatibility
...
2. **Workspace.** `cd "$(npx stim-cli worktree create <name> --carry-ignored)"`.
...
3. **Dev server.** `npx stim-cli start --json`. Assert: correct mode
...
4. **Build.** `npx stim-cli ios` (and/or `android`). Cold: watch the phase
