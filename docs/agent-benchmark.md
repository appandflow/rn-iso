...
The agent creates its own worktree with Git after dispatch:
...
Both arms use the Git branch `worktree-bench/<run-id>`. The Stim arm then runs
`stim worktree warm`; the control copies the same installed dependencies and
native outputs from the fixture main checkout using its own tools.
