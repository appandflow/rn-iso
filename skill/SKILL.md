...
- `npx rn-iso shutdown [-y] [--keep-sims]` — kill **all** Metros, shut down **all** sims/emulators claimed by any rn-iso project, and clear every device assignment. End-of-day reset across every registered project. Prompts unless `-y` / non-TTY; `--keep-sims` only kills Metro and clears assignments without touching the sims. Project entries themselves stay registered, so `metroPort` allocations and labels survive.
